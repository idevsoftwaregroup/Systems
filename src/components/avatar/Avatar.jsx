const Avatar = ({ data }) => {
  ...
  const fileStorage = import.meta.env.VITE_REACT_APP_FILESTORAGE_API_URL;
  const appUrl = "https://app.enjoylife.ir/filestorage/";
  return (
    <div className="flex relative h-screen w-screen min-w-[375px] max-w-[375px] overflow-hidden mx-auto">
      ...
      <div className="absolute inset-x-1/2 inset-y-1/2 bottom-0 transform translate-x-(-50%) h-auto flex flex-col items-center justify-center gap-4 text-stone-300">
        <h3 className="text-4xl text-nowrap">{data.systemName}</h3>
        <div className="text-stone-400 opacity-80 text-nowrap">{data.role}</div>
        <div className="flex justify-center">
          <div className="text-start text-nowrap">کد دستگاه : </div>
          <div className="text-end ms-4 "> {data.systemId} </div>
        </div>
        <div className="flex justify-center">
          <div className="text-start text-nowrap">نام سیستم : </div>
          <div className="text-start ms-4">{data.systemName}</div>
        </div>
        <div className="flex justify-center">
          <div className="text-start text-nowrap">اطلاعات دستگاه : </div>
